...
async function browserOpen(payload) {
  ...
  // Extra wait for JS-heavy SPAs like Gmail
...
async function browserClick(payload) {
  const activePage = await getActivePage();
  if (payload.selector) {
    await activePage.locator(payload.selector).first().click({ timeout: Number(payload.timeout_ms || 20000) });
  } else if (payload.text) {
    await activePage.getByText(payload.text, { exact: !!payload.exact }).first().click({ timeout: Number(payload.timeout_ms || 20000) });
  } else {

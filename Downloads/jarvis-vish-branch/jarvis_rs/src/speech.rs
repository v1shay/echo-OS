...
pub struct OpenAiWhisperSpeechToText {
    api_key: String,
    http: reqwest::Client,
}

impl OpenAiWhisperSpeechToText {
...
#[async_trait]
impl SpeechToText for OpenAiWhisperSpeechToText {
    async fn record_and_transcribe(&self, output_path: &Path, seconds: u64) -> Result<String> {
        let path = output_path.to_path_buf();
        tokio::task::spawn_blocking(move || record_wav(&path, seconds))
            .await
            .context("audio recording join error")??;

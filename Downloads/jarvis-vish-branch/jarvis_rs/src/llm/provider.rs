...
impl OpenAiCompatibleClient {
    ...
    async fn parse_json<T: DeserializeOwned>(&self, system_prompt: &str, user_prompt: &str) -> Result<T> {
        ...
        let raw = body["choices"][0]["message"]["content"]
            ...
            .trim()
            .to_string();
        // Strip markdown code fences that models often add (```json ... ```)
        let content = if let Some(inner) = raw
            .strip_prefix("```json")
        ...
        serde_json::from_str(&content)
            .with_context(|| format!("model {} returned invalid JSON: {}", self.model, raw))
    }
}
...
impl WorkerProvider for HeuristicWorkerProvider {
    async fn next_action(&self, state: &TaskState, _capabilities: &AutomationCapabilities) -> Result<WorkerDecision> {
        ...
        let decision = match step.id.as_str() {
            ...
            _ => {
                // Unknown step IDs should not be handled by the heuristic worker.
                // Return Err so the WorkerStack propagates it and the task fails
                // cleanly instead of looping via infinite Replan.
                bail!(
                    "heuristic worker: no rule for step id '{}' (title: '{}'). Use the API worker.",
                    step.id,
                    step.title
                );
            }
...
impl WorkerProvider for WorkerStack {
    async fn next_action(&self, state: &TaskState, capabilities: &AutomationCapabilities) -> Result<WorkerDecision> {
        match self.primary.next_action(state, capabilities).await {
            ...
            Err(primary_error) => {
                tracing::warn!("Primary worker failed ({}), trying fallback: {:#}", self.primary.provider_name(), primary_error);
                match self.fallback.next_action(state, capabilities).await {
                    Ok(decision) => Ok(decision),
                    Err(fallback_error) => bail!(
                        "worker failed — primary: {} | fallback: {}",
                        primary_error,
                        fallback_error
                    ),
                }

...
use crate::speech::{MacOsSayTextToSpeech, OpenAiWhisperSpeechToText, SpeechToText, TextToSpeech, WhisperCommandSpeechToText};
...
pub fn start(
    ...
    let stt: Arc<dyn SpeechToText> = if let Some(api_key) = config.provider.fallback_api_key.clone() {
        Arc::new(OpenAiWhisperSpeechToText::new(api_key))
    } else {
        Arc::new(WhisperCommandSpeechToText::new(config.whisper_model_path.clone()))
    };
...
fn build_planner_stack(config: &AppConfig) -> Arc<dyn PlannerProvider> {
    let heuristic = Arc::new(HeuristicPlannerProvider) as Arc<dyn PlannerProvider>;
    let (endpoint, model) = if let (Some(url), Some(mdl)) = (
        config.provider.fallback_base_url.as_ref(),
        config.provider.fallback_model.as_ref(),
    ) {
        (url.clone(), mdl.clone())
    } else {
        (config.provider.planner_endpoint.clone(), config.provider.planner_model.clone())
    };
    let api = Arc::new(LocalLlamaPlannerProvider::new(
        endpoint,
        model,
        config.provider.fallback_api_key.clone(),
    )) as Arc<dyn PlannerProvider>;
    // API is primary so OpenAI handles all requests; heuristic is the fallback
    Arc::new(PlannerStack::new(api, heuristic))
}

fn build_worker_stack(config: &AppConfig) -> Arc<dyn WorkerProvider> {
    let heuristic = Arc::new(HeuristicWorkerProvider) as Arc<dyn WorkerProvider>;
    let (endpoint, model) = if let (Some(url), Some(mdl)) = (
        config.provider.fallback_base_url.as_ref(),
        config.provider.fallback_model.as_ref(),
    ) {
        ...
        (config.provider.worker_endpoint.clone(), config.provider.worker_model.clone())
    };
    let api = Arc::new(LocalLlamaWorkerProvider::new(
        endpoint,
        model,
        config.provider.fallback_api_key.clone(),
    )) as Arc<dyn WorkerProvider>;
    // API is primary so OpenAI handles all requests; heuristic is the fallback
    Arc::new(WorkerStack::new(api, heuristic))
}
